 ...
 * @package Modelo
 */
function modelo_widgets_show_widget_field($instance = '', $widget_field = '', $athm_field_value = '') {
    // Store Posts in array
    $modelo_postlist[0] = array(
        'value' => 0,
        'label' => '--choose--'
    );
    $arg = array('posts_per_page' => -1);
    $modelo_posts = get_posts($arg);
    foreach ($modelo_posts as $modelo_post) :
        $modelo_postlist[$modelo_post->ID] = array(
            'value' => $modelo_post->ID,
            'label' => $modelo_post->post_title
        );
    endforeach;

    extract($widget_field);

    switch ($modelo_widgets_field_type) {

        // Standard text field
        case 'text' :
            ?>
            <p>
                <label for="<?php echo $instance->get_field_id($modelo_widgets_name); ?>"><?php echo $modelo_widgets_title; ?>:</label>
                <input class="widefat" id="<?php echo $instance->get_field_id($modelo_widgets_name); ?>" name="<?php echo $instance->get_field_name($modelo_widgets_name); ?>" type="text" value="<?php echo esc_attr($athm_field_value) ; ?>" />

                <?php if (isset($modelo_widgets_description)) { ?>
                    <br />
                    <small><?php echo $modelo_widgets_description; ?></small>
                <?php } ?>
            </p>
            <?php
            break;
        ...
        case 'group_start' :
        ?>
            <div class="main-group" id="ap-font-awesome-list <?php echo $instance->get_field_id(($modelo_widgets_name)); ?>">
                <div class="main-group-heading"><?php echo $modelo_widgets_title ; ?><span class="toogle-arrow"></span></div>
        ...
        case 'url' :
            ...
            <p>
                <label for="<?php echo $instance->get_field_id($modelo_widgets_name); ?>"><?php echo $modelo_widgets_title; ?>:</label>
                <input class="widefat" id="<?php echo $instance->get_field_id($modelo_widgets_name); ?>" name="<?php echo $instance->get_field_name($modelo_widgets_name); ?>" type="text" value="<?php echo $athm_field_value; ?>" />
        ...
        case 'textarea' :
            ...
            <p>
                <label for="<?php echo $instance->get_field_id($modelo_widgets_name); ?>"><?php echo $modelo_widgets_title; ?>:</label>
                <textarea class="widefat" rows="<?php echo $modelo_widgets_row; ?>" id="<?php echo $instance->get_field_id($modelo_widgets_name); ?>" name="<?php echo $instance->get_field_name($modelo_widgets_name); ?>"><?php echo $athm_field_value; ?></textarea>
            </p>
            <?php
            break;
        ...
        case 'checkbox' :
            ?>
            <p>
                <input id="<?php echo $instance->get_field_id($modelo_widgets_name); ?>" name="<?php echo $instance->get_field_name($modelo_widgets_name); ?>" type="checkbox" value="1" <?php checked('1', $athm_field_value); ?>/>
                <label for="<?php echo $instance->get_field_id($modelo_widgets_name); ?>"><?php echo $modelo_widgets_title; ?></label>
        ...
        case 'radio' :
            ...
            <p>
                <?php
                echo $modelo_widgets_title;
                echo '<br />';
                foreach ($modelo_widgets_field_options as $athm_option_name => $athm_option_title) {
                    ?>
                    <input id="<?php echo $instance->get_field_id($athm_option_name); ?>" name="<?php echo $instance->get_field_name($modelo_widgets_name); ?>" type="radio" value="<?php echo $athm_option_name; ?>" <?php checked($athm_option_name, $athm_field_value); ?> />
        ...
        case 'select' :
            ...
            <p>
                <label for="<?php echo $instance->get_field_id($modelo_widgets_name); ?>"><?php echo $modelo_widgets_title; ?>:</label>
                <select name="<?php echo $instance->get_field_name($modelo_widgets_name); ?>" id="<?php echo $instance->get_field_id($modelo_widgets_name); ?>" class="widefat">
                    <?php foreach ($modelo_widgets_field_options as $athm_option_name => $athm_option_title) { ?>
        ...
        case 'number' :
            ?>
            <p>
                <label for="<?php echo $instance->get_field_id($modelo_widgets_name); ?>"><?php echo $modelo_widgets_title; ?>:</label><br />
                <input name="<?php echo $instance->get_field_name($modelo_widgets_name); ?>" type="number" step="4" min="4" id="<?php echo $instance->get_field_id($modelo_widgets_name); ?>" value="<?php echo $athm_field_value; ?>" class="small-text" />
        ...
        case 'selectpost' :
            ...
            <p>
                <label for="<?php echo $instance->get_field_id($modelo_widgets_name); ?>"><?php echo $modelo_widgets_title; ?>:</label>
                <select name="<?php echo $instance->get_field_name($modelo_widgets_name); ?>" id="<?php echo $instance->get_field_id($modelo_widgets_name); ?>" class="widefat">
                    <?php foreach ($modelo_postlist as $modelo_single_post) { ?>
                        <option value="<?php echo $modelo_single_post['value']; ?>" id="<?php echo $instance->get_field_id($modelo_single_post['label']); ?>" <?php selected($modelo_single_post['value'], $athm_field_value); ?>><?php echo $modelo_single_post['label']; ?></option>
                    <?php } ?>
                </select>
        ...
        case 'upload' :
            ...
            $output = '';
            $id = $instance->get_field_id($modelo_widgets_name);
            $class = '';
            $int = '';
            $value = $athm_field_value;
            $name = $instance->get_field_name($modelo_widgets_name);
            ...
            $output .= '<label for="'.$instance->get_field_id($modelo_widgets_name).'">'.$modelo_widgets_title.'</label><br/>';
            $output .= '<input id="' . $id . '" class="upload' . $class . '" type="text" name="' . $name . '" value="' . $value . '" placeholder="' . __('No file chosen', 'modelo') . '" />' . "\n";
            if (function_exists('wp_enqueue_media')) {
                if (( $value == '')) {
                    $output .= '<input id="upload-' . $id . '" class="upload-button-wdgt button" type="button" value="' . __('Upload', 'modelo') . '" />' . "\n";
                } else {
                    $output .= '<input id="remove-' . $id . '" class="remove-file button" type="button" value="' . __('Remove', 'modelo') . '" />' . "\n";
                }
            } else {
                $output .= '<p><i>' . __('Upgrade your version of WordPress for full media support.', 'modelo') . '</i></p>';
            }
            ...
            <p>
                ...
                <input class="hidden-icon-input" name="<?php echo $instance->get_field_name($modelo_widgets_name); ?>" type="hidden" id="<?php echo $instance->get_field_id($modelo_widgets_name); ?>" value="<?php echo $athm_field_value; ?>" />
...
function modelo_widgets_updated_field_value($widget_field, $new_field_value) {

    extract($widget_field);

    // Allow only integers in number fields
    if ($modelo_widgets_field_type == 'number') {
        return absint($new_field_value);

        // Allow some tags in textareas
    } elseif ($modelo_widgets_field_type == 'textarea') {
        // Check if field array specifed allowed tags
        if (!isset($modelo_widgets_allowed_tags)) {
            // If not, fallback to default tags
            $modelo_widgets_allowed_tags = '<p><strong><em><a>';
        }
        
        return strip_tags($new_field_value, $modelo_widgets_allowed_tags);

        // No allowed tags for all other fields
    } elseif ($modelo_widgets_field_type == 'iframe_textarea') {
        // Check if field array specifed allowed tags
        if (!isset($modelo_widgets_allowed_tags)) {
            // If not, fallback to default tags
            $modelo_widgets_allowed_tags = '<iframe>';
        }
    ...
    elseif ($modelo_widgets_field_type == 'url') {
        return esc_url_raw($new_field_value);
    }
    elseif ($modelo_widgets_field_type == 'title') {
...
function optionsframework_media_scriptss($hook) {
    ...
    wp_localize_script('of-media-uploader', 'optionsframework_l10n', array(
        'upload' => __('Upload', 'modelo'),
        'remove' => __('Remove', 'modelo')
    ));
